...
function createComment(url, request) {
  ...
  const comBody = makeComment && makeComment.body;
  const comUsername = makeComment && makeComment.username;
  const comArticleId = makeComment && makeComment.articleId;
  const response = {};

  if (comBody && comArticleId && database.users[comUsername] && database.articles[comArticleId]){
      const newComment = {
        id: database.nextCommentId++,
        username: comUsername,
        articleId: comArticleId,
      ...
      database.articles[comArticleId].commentIds.push(newComment.id);
...
function updateComment (url, request) {
  ...
  const existComment = database.comments[id];
  const pullComment = request.body && request.body.comment;
  const response = {};
  ...
  if (!id || !pullComment){
    ...
    existComment.body = pullComment.body || existComment.body;
    existComment.url = pullComment.url || existComment.url;

    response.body = {comment: existComment};
    response.status = 200;
    }
  return response;
};
...
function deleteComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const delComment = database.comments[id];
  const response = {};

  if (delComment) {
    const userCommentIds = database.users[delComment.username].commentIds;
    userCommentIds.splice(userCommentIds.indexOf(id), 1);

    const articleDelCommentIds = database.articles[delComment.articleId].commentIds;
    articleDelCommentIds.splice(articleDelCommentIds.indexOf(id), 1);
...
//- DELETE
  ...
  //- PUT
    ...
    //- If no ID is supplied, comment with supplied ID doesn't exist, or user
    //with supplied username doesn't exist, returns 400 response
...
//- PUT
  //- Receives comment ID from URL parameter and username from `username`
  //property of request body
  //- Adds supplied username to `downvotedBy` of corresponding comment if
  //user hasn't already downvoted the comment, remove username from `upvotedBy`
  //if that user had previously upvoted the comment, returns 200 response with
